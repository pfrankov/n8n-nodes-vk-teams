import { booleanParam, nonemptyString } from './message.options';

type Input = Record<string, unknown>;
function subscribers(input: Input): Record<string, unknown> {
	const params: Record<string, unknown> = { threadId: nonemptyString(input.threadId, 'threadId') };
	...
	if (input.cursor !== undefined && input.cursor !== '')
		params.cursor = nonemptyString(input.cursor, 'cursor');
...
> = {
	'thread.add': {
		...
		params: (input) => ({
			...
			msgId: nonemptyString(input.msgId, 'msgId'),
		}),
	},
