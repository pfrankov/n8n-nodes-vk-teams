...
Наличие в отдельном SDK не заменяет контракт: changedChatInfo пока не имеет проверенного payload; sendTextWithDeeplink отсутствует в публичном каталоге; request-id официально описан для логов, а не как гарантия идемпотентности. Исходящий parent_topic и серверная семантика удаления клавиатуры по [] не домысливаются. С последующего расширения `createChat`/`members/add` поддерживаются отдельно и явно помечены как методы с ограниченной доступностью. FSM/планировщики SDK не дублируются внутри n8n.
