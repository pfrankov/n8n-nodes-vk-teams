...
В том же workflow есть отключённые и не подключённые примеры `Example createChat (private)` и `Example addMembers (private)`. Оба метода в официальной документации помечены `myteam_only`/`privateMethod`, поэтому проверяйте их только там, где администратор разрешил тестирование; наличие операции в n8n не гарантирует поддержку сервером.

1. Для создания чата укажите отдельное тестовое название, назначьте credentials, включите и подключите только `Example createChat (private)` к отдельному Manual Trigger. Запустите один раз; при успехе сохраните `sn` из ответа. Не повторяйте запрос автоматически: повторное выполнение может создать ещё один чат.
2. Для добавления участников укажите `sn` тестового чата как `Chat ID` и одного согласованного тестового пользователя в `Members (JSON)`. Подключите только `Example addMembers (private)` и запустите один раз. Сверьте фактический состав чата и массив `failures` в ответе; успешный HTTP-ответ может содержать частичные отказы.
3. При отказе сервера сохраните текст ошибки и проверьте версию/права с администратором. Нода не переключается на альтернативный endpoint и не выдаёт права сама. После проверки отключите оба примера; тестовый чат удаляйте только по правилам вашей инсталляции.
