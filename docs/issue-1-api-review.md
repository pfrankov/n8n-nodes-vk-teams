...
Перечисленные автором методы существуют в официальной спецификации и SDK; оснований считать их выдуманными нет. Ошибка — объединение общих методов и private/on-premise возможностей в обещание полного публичного покрытия. Для автоматизаций состава и модерации существующих чатов расширение полезно не только автору запроса. Создание чата с автоматическим добавлением участников в этот PR не входит.
...
| --- | --- | --- |
| `chats/createChat` | Официальная спецификация: `myteam_only`/`privateMethod`; Python `myteam.py`; async SDK | Не добавлять в общий интерфейс |
| `chats/members/add` | Официальная спецификация: `myteam_only`/`privateMethod`; Python `myteam.py`; async SDK | Не добавлять в общий интерфейс |
...
Нет автоматического обхода страниц, повторов изменяющих запросов, синхронизации состава, обхода прав, скрытого выбора всех участников, смены credentials или внешних получателей. Удаление последних сообщений отключено по умолчанию. Аватар загружается из входного binary, а не по произвольному URL. В итоговом diff не меняются версия пакета, публикация npm, workflow CI и настройки репозитория.
