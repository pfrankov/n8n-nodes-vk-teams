...
- `chat.getInfo`
- `chat.createChat` (доступность зависит от сервера)
- `chat.addMembers` (доступность зависит от сервера)
- `chat.getMembers`
- `chat.getAdmins`
...
`Chat` → `Create Chat` создаёт новый чат по обязательному `Name`; `Chat ID` для этой операции не нужен. По желанию заполните `About`, `Rules`, `Members (JSON)`, `Public`, `Default Role` и `Join Moderation`. По умолчанию список участников пуст, чат непубличный, модерация вступления выключена, роль — `member`. `Default Role` остаётся строкой: официальная спецификация не перечисляет допустимые значения, поэтому их проверяет сервер. ID нового чата находится в поле `sn` ответа.

`Chat` → `Add Members` принимает `Chat ID` и непустой `Members (JSON)` — массив уникальных строковых ID, например `["user1@example.com", "user2@example.com"]`. Этот же формат используется для необязательных участников при создании чата. Нода превращает список в ожидаемые API объекты `{"sn":"..."}`. При частичном результате проверьте `failures` в ответе, даже если `ok=false`: это список отдельных отказов, а не подтверждение добавления каждого участника. Общая ошибка API остаётся ошибкой ноды.

Оба метода отмечены в официальной документации как `myteam_only` и `privateMethod`. Они показаны в интерфейсе с предупреждением, но могут не работать на конкретной инсталляции или для конкретного бота. Нода не обходит серверные ограничения и не повторяет изменяющий запрос автоматически; для проверки используйте отдельный тестовый чат и [ручной сценарий](docs/workflows/README.md#создание-чата-и-добавление-участников).
...
Доступность операций зависит от версии сервера и прав бота. Разбор источников и границ покрытия: [исследование issue #1](docs/issue-1-api-review.md). Поддержка методов в ноде не означает, что они доступны каждому боту.
...
- Методы `chats/createChat` и `chats/members/add` доступны в ноде, но помечены `myteam_only`/`privateMethod`; поддержка и права на конкретном сервере не гарантированы. Нода не выдаёт боту дополнительных прав.
